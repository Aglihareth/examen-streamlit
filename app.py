        ...
        if st.button("📨 Entregar examen", type="primary"):
            if len(respuestas) < len(preguntas):
                ...
                try:
                    ...
                    doc_bytes = generar_reporte(
                        nombre, materia, resultado, preguntas
                    )
...
# ── PANTALLA DE RESULTADOS ──
    ...
    st.success(f"¡Gracias, *{st.session_state.nombre}*!")

    col1, col2, col3 = st.columns(3)
    col1.metric("Puntos obtenidos", resultado['calificacion'])
    col2.metric("Total posible", resultado['total'])
    col3.metric("Porcentaje",
        f"{round((resultado['calificacion']/resultado['total'])*100,1)}%")

    st.info("⚠️ Las preguntas abiertas serán revisadas por tu profesor.")
    st.warning("⬇️ Descarga tu reporte y envíalo a tu profesor como evidencia")

    st.download_button(
        label="📄 DESCARGAR MI REPORTE",
        data=st.session_state.doc_bytes,
        file_name=f"Examen_{st.session_state.nombre}_{st.session_state.materia}.docx",
